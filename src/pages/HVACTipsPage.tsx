...
const categoryColors: Record<string, string> = {
  "Yard Care": "bg-green-500/10 text-green-400 border-green-500/30",
  "Property Management": "bg-blue-500/10 text-blue-400 border-blue-500/30",
  "Property Preservation": "bg-purple-500/10 text-purple-400 border-purple-500/30",
  "Tree & Shrub Care": "bg-emerald-500/10 text-emerald-400 border-emerald-500/30",
  "Property Value": "bg-amber-500/10 text-amber-400 border-amber-500/30",
  "Storm Prep": "bg-red-500/10 text-red-400 border-red-500/30",
};

const HVACTipsPage = () => {
  useSEO({
    title: "Property Care Tips & Guides | PricedRight Yards & More",
    description: "Expert property care tips. Learn about yard maintenance, property preservation, tree care, and rental property management.",
          ...
          <div className="container mx-auto px-4 lg:px-8 py-20 lg:py-32 relative z-10 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="flex items-center justify-center gap-2 mb-4">
                ...
                <span className="text-sm font-semibold text-secondary uppercase tracking-wider">Expert Advice</span>
              </div>
              <h1 className="font-heading text-4xl lg:text-6xl font-bold text-primary-foreground">Property Care Tips & Guides</h1>
              <p className="text-lg text-primary-foreground/70 max-w-2xl mx-auto mt-6 leading-relaxed">Practical advice from our experienced team to help you maintain your property, increase its value, and save money.</p>
            </motion.div>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {propertyTips.map((tip, i) => (
                <motion.a key={tip.slug} href={`/tips/${tip.slug}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group rounded-2xl p-8 border border-secondary/30 hover:border-secondary/60 transition-all" style={{ backgroundColor: "hsl(148 40% 12%)" }}>
                  ...
                  <h2 className="font-heading text-xl font-bold text-primary-foreground mb-3 group-hover:text-secondary transition-colors">{tip.title}</h2>
                  <p className="text-sm text-primary-foreground/70 leading-relaxed mb-4">{tip.excerpt}</p>
                  <span className="inline-flex items-center gap-2 text-sm font-semibold text-secondary group-hover:gap-3 transition-all">Read More <ArrowRight className="w-4 h-4" /></span>
                </motion.a>
              ))}
            </div>
          </div>
